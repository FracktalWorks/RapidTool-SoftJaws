...
- [x] **Shallow conforming pocket — vertical channel removed.** The cut now conforms to the workpiece silhouette ONLY where the part overlaps the jaw's own height; it no longer opens a vertical channel up to the jaw top. Both the primary 3D-conforming path and the prismatic fallback in `profileWorker.ts` dropped their `+Y` sweep / slot-quad logic (the `Dynamix Y-Clipping` behaviour the user flagged as wrong vs Trinckle). Pocket depth is derived from the workpiece (`partSpanX / 2`, i.e. each jaw wraps to the part's X-midline so the two jaws fully enclose it, clamped to `jawThickness − 2 mm` wall) instead of the fixed `depth` param. The slab Y-range is `[bb.minY, bb.maxY]` (jaw height + 1 mm each end) so anything the workpiece does above the jaw top stays in open air, uncut.
